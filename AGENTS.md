...
## Time and lifecycle
Domain logic receives explicit timestamps and balance configuration and never reads a system clock. Foreground time comes from Application's monotonic anchor; offline/resume time comes from Application wall-clock reconciliation. Durable event IDs are explicit; deterministic simulation IDs derive only from simulation coordinates, while future non-deterministic IDs originate outside Domain. Preserve egg/pet distinction, immutable pet identity and birth time, and the invalid `DEAD → ALIVE` transition.
