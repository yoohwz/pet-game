...
For an ALIVE pet, activity is independent of life/growth state: `AWAKE` has no sleep timestamp; `SLEEPING` has a persisted start timestamp. Survival is `STABLE` or `CRITICAL`; rescue clears CRITICAL only once both hunger and hydration are above zero, without health recovery. CRITICAL remains visibly represented even while the pet is sleeping, although sleep still exposes Wake only. DEAD is permanent and frozen. A dead pet is memorialized only by an explicit candidate transaction; a replacement egg is likewise explicit and preserves all memorials.
