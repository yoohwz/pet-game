...
The local JSON ruleset recognizes bounded English/Vietnamese phrases with deterministic priority, canonical topic ordering, integer sentiment and non-verbal reaction keys. `LanguageModel` is pure Domain code: it receives text, memory and rules and has no clock, filesystem, UI or network dependency. It preserves original accepted text in events while normalized text is used for matching.
