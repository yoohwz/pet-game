...
func _run() -> void:
	...
	eq(none.schema_version, 6, "new profile uses schema v6")
	...
	eq(migrated_inc.schema_version, 6, "v3 incubating migrates v6")
	...
	eq(migrated_v4.schema_version, 6, "v4 pet migrates to v6")
	...
	var v5_inc := SaveMigratorScript.migrate(v4_egg); eq(v5_inc.schema_version, 6, "v4 incubating migrates to v6"); eq(v5_inc.active_egg.egg_id, "egg:v4", "v4 egg id preserved"); eq(v5_inc.active_egg.hatch_ready_at, 15400, "v4 egg ready time preserved")
	...
	for egg_state in ["INCUBATING", "READY", "HATCHING"]:
		...
		var migrated_v5_egg: Dictionary = SaveMigratorScript.migrate(v5_egg_profile); eq(migrated_v5_egg.schema_version, 6, "v5 %s root migrates v6" % egg_state); eq(migrated_v5_egg.active_egg.state, egg_state, "v5 %s egg state preserved" % egg_state); eq(migrated_v5_egg.active_egg.egg_id, v5_egg_profile.active_egg.egg_id, "v5 %s egg identity preserved" % egg_state)
	...
	for value in [relation_clamp_session, rescue_clamp, low_energy, v2_failure, care_authority, rescue_authority, sleep_memory_session]: value.free()
