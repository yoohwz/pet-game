...
func _run() -> void:
	...
	eq(relationship_file_config.relationship_balance_version, 1.0, "relationship config declares v1"); eq(relationship_file_config.reward_cooldown_seconds, 300.0, "relationship config declares cooldown"); eq(relationship_file_config.rewards.touch.bond, 2.0, "relationship config declares touch reward"); eq(relationship_file_config.rescue.trust, 5.0, "relationship config declares rescue reward"); eq(memory_file_config.memory_version, 2.0, "memory config declares v2"); eq(memory_file_config.event_store_limit, 64.0, "memory config declares event cap"); eq(memory_file_config.working_memory_limit, 8.0, "memory config declares window"); eq(memory_file_config.episodic_min_importance, 3.0, "memory config declares episodic threshold")
...
func _run_phase7_language_tests(lifecycle: Dictionary, care: Dictionary, survival: Dictionary, relationship: Dictionary, memory: Dictionary, growth: Dictionary) -> void:
	...
	var vietnamese: Dictionary = LanguageModelScript.understand("Xin chao ban", direct_memory, rules); eq(vietnamese.intent, "GREETING", "Vietnamese greeting recognized"); ok("PET" in vietnamese.topics, "Vietnamese greeting derives canonical pet topic")
	for rule_value in rules.get("intent_rules", []):
		var rule: Dictionary = rule_value; var phrases: Array = rule.get("phrases", []); eq(LanguageModelScript.understand(String(phrases[0]), direct_memory, rules).intent, String(rule.intent), "%s English rule recognizes deterministically" % rule.intent); eq(LanguageModelScript.understand(String(phrases[-1]), direct_memory, rules).intent, String(rule.intent), "%s Vietnamese rule recognizes deterministically" % rule.intent)
	...
	var events_before_reject: Array = spoken.profile.recent_events.duplicate(true); eq(spoken.speak_to_pet("   ", 10.0).reason, "EMPTY_MESSAGE", "empty message rejected"); eq(spoken.speak_to_pet("x".repeat(257), 10.0).reason, "MESSAGE_TOO_LONG", "overlong message rejected"); eq(spoken.profile.recent_events, events_before_reject, "rejected messages append no events")
	...
	var failed := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules); var failed_events: Array = failed.profile.recent_events.duplicate(true); var failed_memory: Dictionary = failed.profile.active_pet.memory.duplicate(true); LocalSaveRepositoryScript.test_fail_next_primary_replace = true; var failed_result: Dictionary = failed.speak_to_pet("hello pet", 10.0); eq(failed_result.reason, "PERSIST_FAILED", "language candidate failure reports persistence error"); eq(failed.profile.recent_events, failed_events, "failed language candidate adds no authoritative event"); eq(failed.profile.active_pet.memory, failed_memory, "failed language candidate adds no authoritative memory")
	var v7_profile: Dictionary = fixture(); v7_profile.schema_version = 7; v7_profile.active_pet.schema_version = 7; v7_profile.active_pet.memory.memory_version = 1; v7_profile.active_pet.memory.semantic.erase("language"); var migrated_v7: Dictionary = SaveMigratorScript.migrate(v7_profile); ok(DomainStateScript.validate_profile(migrated_v7), "v7 pet migrates to valid v8 language memory"); eq(migrated_v7.active_pet.memory.memory_version, 2, "v7 migration upgrades memory schema"); eq(migrated_v7.active_pet.memory.semantic.language.message_count, 0, "v7 migration initializes language projection without backfill")
	var v7_dead_snapshot: Dictionary = fixture().active_pet; v7_dead_snapshot.schema_version = 7; v7_dead_snapshot.memory.memory_version = 1; v7_dead_snapshot.memory.semantic.erase("language"); v7_dead_snapshot.life.life_state = "DEAD"; v7_dead_snapshot.life.died_at = 1000; v7_dead_snapshot.life.death_cause = "STARVATION"; v7_dead_snapshot.vitals.health = 0.0
	...
	var v7_hatching: Dictionary = DomainStateScript.new_profile("profile:v7-language-egg", 1000); v7_hatching.schema_version = 7; v7_hatching.active_subject = "EGG"; v7_hatching.active_egg = DomainStateScript.new_egg("egg:v7-language", 1000, 15400); v7_hatching.active_egg.schema_version = 7; v7_hatching.active_egg.state = "HATCHING"; v7_hatching.active_egg.reserved_pet_id = "pet:v7-language"; v7_hatching.active_egg.reserved_pet_seed = 7; v7_hatching.active_egg.hatching_started_at = 15400; var migrated_v7_hatching: Dictionary = SaveMigratorScript.migrate(v7_hatching); eq(migrated_v7_hatching.active_egg.reserved_pet_id, "pet:v7-language", "v7 hatching preserves reserved identity")
	...
	var malformed_payload: Dictionary = spoken.profile.duplicate(true); malformed_payload.active_pet.memory.events[0].details.sentiment = 2; ok(not DomainStateScript.validate_profile(malformed_payload), "invalid retained language sentiment fails validation")
	var memory_model = preload("res://domain/memory/memory_model.gd"); var bounded_language: Dictionary = memory_model.new_memory(); bounded_language = memory_model.project(bounded_language, {"event_id":"evt:language:hatch","event_type":"pet_hatched","occurred_at":1,"payload":{}}, memory)
	...
	var ui_session := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules); var ui := FoundationScreenScript.new(); ui.session_override = ui_session; ui._ready(); var feed := ui.lifecycle_button("Feed"); ok(ui.language_input.visible and ui.language_send.visible, "awake pet exposes stable local language controls"); ui.language_input.text = "hello pet"; ui._speak(); ok(ui.language_diagnostics.text.contains("GREETING"), "language UI renders deterministic diagnostic result"); eq(ui.lifecycle_button("Feed"), feed, "language refresh does not rebuild care controls")
	ui_session.profile.active_pet.activity = {"state":"SLEEPING","sleep_started_at":1000}; ui.refresh(); ok(not ui.language_input.visible and not ui.language_send.visible, "sleeping pet hides language controls"); ui_session.profile.active_pet.activity = {"state":"AWAKE","sleep_started_at":null}; ui.refresh(); ok(ui.language_input.visible, "awake transition restores language input")
	ui.free(); ui_session.free(); language_cycle.free(); critical.free(); no_pet.free(); failed.free(); spoken.free()
