...
func _run() -> void:
	...
	eq(none.schema_version, 8, "new profile uses schema v8")
	...
	eq(migrated_inc.schema_version, 8, "v3 incubating migrates v8")
	...
	eq(migrated_v4.schema_version, 8, "v4 pet migrates to v8")
	...
	var v5_inc := SaveMigratorScript.migrate(v4_egg); eq(v5_inc.schema_version, 8, "v4 incubating migrates to v8"); eq(v5_inc.active_egg.egg_id, "egg:v4", "v4 egg id preserved"); eq(v5_inc.active_egg.hatch_ready_at, 15400, "v4 egg ready time preserved")
	...
	var MEMORY := {"memory_version":2,"event_store_limit":64,"working_memory_limit":8,"episodic_min_importance":3}
	...
	for egg_state in ["INCUBATING", "READY", "HATCHING"]:
		...
		var migrated_v5_egg: Dictionary = SaveMigratorScript.migrate(v5_egg_profile); eq(migrated_v5_egg.schema_version, 8, "v5 %s root migrates v8" % egg_state); eq(migrated_v5_egg.active_egg.state, egg_state, "v5 %s egg state preserved" % egg_state); eq(migrated_v5_egg.active_egg.egg_id, v5_egg_profile.active_egg.egg_id, "v5 %s egg identity preserved" % egg_state)
	...
	var relationship_config_file := FileAccess.open("res://data/balance/relationship_v1.json", FileAccess.READ); var relationship_file_config: Dictionary = JSON.parse_string(relationship_config_file.get_as_text()); relationship_config_file.close(); var memory_v1_file := FileAccess.open("res://data/config/memory_v1.json", FileAccess.READ); var memory_v1_file_config: Dictionary = JSON.parse_string(memory_v1_file.get_as_text()); memory_v1_file.close(); var memory_config_file := FileAccess.open("res://data/config/memory_v2.json", FileAccess.READ); var memory_file_config: Dictionary = JSON.parse_string(memory_config_file.get_as_text()); memory_config_file.close()
	eq(relationship_file_config.relationship_balance_version, 1.0, "relationship config declares v1"); eq(relationship_file_config.reward_cooldown_seconds, 300.0, "relationship config declares cooldown"); eq(relationship_file_config.rewards.touch.bond, 2.0, "relationship config declares touch reward"); eq(relationship_file_config.rescue.trust, 5.0, "relationship config declares rescue reward"); eq(memory_v1_file_config.memory_version, 1.0, "memory v1 resource remains honest"); eq(memory_file_config.memory_version, 2.0, "memory v2 config declares v2"); eq(memory_file_config.event_store_limit, 64.0, "memory config declares event cap"); eq(memory_file_config.working_memory_limit, 8.0, "memory config declares window"); eq(memory_file_config.episodic_min_importance, 3.0, "memory config declares episodic threshold")
	# Phase 5 corrective pass 2: strict version validation and exact JSON canonicalization.
	for memory_version_case in [2, 1.5, "2", true, 1, null]:
		var memory_version_pet: Dictionary = fixture().active_pet; memory_version_pet.memory.memory_version = memory_version_case; eq(DomainStateScript.validate_pet(memory_version_pet), memory_version_case is int and memory_version_case == 2, "memory version strict validation for %s" % str(memory_version_case))
	var whole_external := fixture(); whole_external.active_pet.memory.memory_version = 2.0; whole_external.active_pet.memory.next_sequence = 1.0; whole_external.active_pet.memory.events = [{"memory_id":"memory:whole","source_event_id":"evt:whole","sequence":0.0,"event_type":"pet_fed","occurred_at":1000.0,"category":"CARE","valence":1.0,"importance":1.0,"details":{}}]; whole_external.active_pet.memory.routine.feed = {"count":1.0,"meaningful_count":1.0,"last_at":1000.0}; whole_external.active_pet.memory.semantic.care_interaction_count = 1.0; whole_external.active_pet.relationship.relationship_version = 1.0; whole_external.active_pet.relationship.relationship_balance_version = 1.0; whole_external.active_pet.relationship.last_rewarded_at.feed = 1000.0
	...
	eq(none.simulation.simulation_version, 6, "new profile uses simulation v6"); eq(none.simulation.growth_balance_version, 1, "new profile records growth balance version"); eq(p.active_pet.schema_version, 8, "new pet uses schema v8"); eq(p.active_pet.life.growth_stage, "NEWBORN", "new pet starts newborn"); eq(p.active_pet.growth.stage_started_at, p.active_pet.identity.born_at, "newborn growth starts at birth"); ok(DomainStateScript.validate_pet(p.active_pet), "new pet growth metadata validates")
	...
	ok(DomainEventScript.is_valid(child_growth[0]), "pet_grew is a valid canonical DomainEvent"); eq(child_growth[0].schema_version, 1, "pet_grew uses event schema v1"); eq(child_growth[0].subject_id, "pet:test", "pet_grew subject is the pet identity"); eq(child_growth[0].event_type, "pet_grew", "pet_grew event type is canonical")
	var growth_ids := {}
	for grown_event in long_events:
		ok(DomainEventScript.is_valid(grown_event), "every multi-stage growth event is a valid DomainEvent"); ok(not growth_ids.has(grown_event.event_id), "multi-stage growth event IDs are unique"); growth_ids[grown_event.event_id] = true
	...
	# Application projection must have the same lifecycle-memory history whether
	# a long offline gap is reconciled once or in several production advances.
	var growth_memory_chunks = PetGameSessionScript.new(); growth_memory_chunks.balance = BALANCE; growth_memory_chunks.lifecycle = care_session.lifecycle; growth_memory_chunks.care = care_session.care; growth_memory_chunks.survival = SURVIVAL; growth_memory_chunks.relationship_balance = RELATIONSHIP; growth_memory_chunks.memory_config = MEMORY; growth_memory_chunks.growth = GROWTH; growth_memory_chunks.profile = long_growth.duplicate(true)
	for application_growth_time in [1000 + 86400, 1000 + 3 * 86400, 1000 + 10 * 86400, 1000 + 30 * 86400]: growth_memory_chunks.advance_in_memory_to(application_growth_time)
	var application_long_growth_memories: Array = _memory_events_by_types(memory_long.profile.active_pet.memory.events, ["pet_grew"]); var application_chunked_growth_memories: Array = _memory_events_by_types(growth_memory_chunks.profile.active_pet.memory.events, ["pet_grew"])
	eq(application_chunked_growth_memories, application_long_growth_memories, "application growth memory chunking is deterministic"); eq(growth_memory_chunks.profile.active_pet.growth, memory_long.profile.active_pet.growth, "application chunking preserves final growth metadata")
	var long_growth_memory_count: int = application_long_growth_memories.size(); var chunk_growth_memory_count: int = application_chunked_growth_memories.size(); memory_long.advance_in_memory_to(1000 + 31 * 86400); growth_memory_chunks.advance_in_memory_to(1000 + 31 * 86400); eq(_memory_events_by_types(memory_long.profile.active_pet.memory.events, ["pet_grew"]).size(), long_growth_memory_count, "adult application reconciliation adds no growth memory"); eq(_memory_events_by_types(growth_memory_chunks.profile.active_pet.memory.events, ["pet_grew"]).size(), chunk_growth_memory_count, "chunked adult reconciliation adds no growth memory"); eq(_unique_memory_source_count(_memory_events_by_types(memory_long.profile.active_pet.memory.events, ["pet_grew"])), long_growth_memory_count, "growth memory source IDs remain exact-once")
	...
	var predeath_memory_session = PetGameSessionScript.new(); predeath_memory_session.balance = BALANCE; predeath_memory_session.lifecycle = care_session.lifecycle; predeath_memory_session.care = care_session.care; predeath_memory_session.survival = SURVIVAL; predeath_memory_session.relationship_balance = RELATIONSHIP; predeath_memory_session.memory_config = MEMORY; predeath_memory_session.growth = GROWTH; predeath_memory_session.profile = growth_before_death.duplicate(true); predeath_memory_session.advance_in_memory_to(200000); eq(_memory_events_by_types(predeath_memory_session.profile.active_pet.memory.events, ["pet_grew"]).size(), 0, "death before threshold projects no growth memory")
	...
	var same_second: Dictionary = SimulationKernelScript.simulate(growth_before_death, 1000, 11800, BALANCE, {}, care_session.care, SURVIVAL, same_second_growth); var same_second_grew: Dictionary = _events_by_type(same_second.generated_events, "pet_grew")[0]; var same_second_died: Dictionary = _events_by_type(same_second.generated_events, "pet_died")[0]; eq(same_second_grew.occurred_at, 8200, "same-second growth has exact death timestamp"); eq(same_second_died.occurred_at, 8200, "same-second death has exact growth timestamp"); eq(same_second.new_state.active_pet.life.life_state, "DEAD", "same-second collision ends dead"); eq(same_second.new_state.active_pet.life.growth_stage, "CHILD", "growth at death second is retained"); eq(same_second.new_state.active_pet.growth.stage_started_at, 8200, "same-second child stage starts at collision timestamp"); ok(_event_index(same_second.generated_events, "pet_grew") < _event_index(same_second.generated_events, "pet_died"), "same-second growth precedes death deterministically")
	...
	# A real grown pet keeps its growth history through death, memorialization and
	# a replacement egg cycle. The new pet starts with no inherited growth state.
	_reset(); var grown_cycle = PetGameSessionScript.new(); grown_cycle.balance = BALANCE; grown_cycle.lifecycle = care_session.lifecycle; grown_cycle.care = care_session.care; grown_cycle.survival = SURVIVAL; grown_cycle.relationship_balance = RELATIONSHIP; grown_cycle.memory_config = MEMORY; grown_cycle.growth = GROWTH; grown_cycle.profile = long_growth.duplicate(true); grown_cycle.reanchor(10.0); grown_cycle.advance_in_memory_to(173800); grown_cycle.profile.active_pet.life.newborn_protection_until = 0; grown_cycle.profile.active_pet.vitals.hunger = 0.0; grown_cycle.profile.active_pet.vitals.hydration = 0.0; grown_cycle.profile.active_pet.vitals.health = 12.0; grown_cycle.advance_in_memory_to(182000)
	eq(grown_cycle.profile.active_pet.life.life_state, "DEAD", "grown production pet reaches deterministic death"); eq(grown_cycle.profile.active_pet.life.growth_stage, "CHILD", "growth before death remains in dead profile"); var grown_dead_growth: Dictionary = grown_cycle.profile.active_pet.growth.duplicate(true); var grown_dead_memory: Dictionary = grown_cycle.profile.active_pet.memory.duplicate(true); var grown_pet_id := String(grown_cycle.profile.active_pet.identity.pet_id); ok(LocalSaveRepositoryScript.save_profile(grown_cycle.profile), "persist grown dead pet before memorial")
	# Application paths must freeze a dead pet with previously projected growth.
	var dead_growth_session = PetGameSessionScript.new(); dead_growth_session.balance = BALANCE; dead_growth_session.lifecycle = care_session.lifecycle; dead_growth_session.care = care_session.care; dead_growth_session.survival = SURVIVAL; dead_growth_session.relationship_balance = RELATIONSHIP; dead_growth_session.memory_config = MEMORY; dead_growth_session.growth = GROWTH; dead_growth_session.profile = grown_cycle.profile.duplicate(true); dead_growth_session.reanchor(10.0); var dead_growth_memory_count: int = _memory_events_by_types(dead_growth_session.profile.active_pet.memory.events, ["pet_grew"]).size()
	dead_growth_session.resume_at(182000 + 86400, 20.0); dead_growth_session.advance_debug(604800, 30.0); dead_growth_session.advance_debug(365 * 86400, 40.0); dead_growth_session.resume_at(1000, 50.0)
	eq(dead_growth_session.profile.active_pet.life.growth_stage, "CHILD", "dead application paths retain growth stage"); eq(dead_growth_session.profile.active_pet.growth, grown_dead_growth, "dead application paths retain growth metadata"); eq(dead_growth_session.profile.active_pet.memory, grown_dead_memory, "dead application paths retain growth memory"); eq(_memory_events_by_types(dead_growth_session.profile.active_pet.memory.events, ["pet_grew"]).size(), dead_growth_memory_count, "dead application paths append no growth memory")
	ok(grown_cycle.memorialize_pet(10.0).ok, "grown dead pet memorializes successfully"); var grown_memorial: Dictionary = grown_cycle.profile.memorials[-1].duplicate(true); eq(grown_memorial.pet_snapshot.life.growth_stage, "CHILD", "memorial snapshot preserves grown stage"); eq(grown_memorial.pet_snapshot.growth, grown_dead_growth, "memorial snapshot preserves growth metadata"); eq(grown_memorial.pet_snapshot.memory, grown_dead_memory, "memorial snapshot preserves growth memories")
	ok(grown_cycle.request_new_egg(20.0).ok, "replacement egg is explicitly issued after grown memorial"); eq(grown_cycle.profile.active_egg.state, "INCUBATING", "replacement begins incubating"); grown_cycle.advance_debug(14400, 20.0); ok(grown_cycle.begin_hatching(int(grown_cycle.profile.simulation.last_simulated_at), 20.0), "replacement hatching begins"); ok(grown_cycle.complete_hatching(int(grown_cycle.profile.simulation.last_simulated_at), 20.0), "replacement hatching completes")
	eq(String(grown_cycle.profile.active_pet.identity.pet_id) != grown_pet_id, true, "replacement pet has a different identity"); eq(grown_cycle.profile.active_pet.life.growth_stage, "NEWBORN", "replacement pet resets to newborn"); eq(grown_cycle.profile.active_pet.growth.growth_version, 1, "replacement pet uses growth v1"); eq(grown_cycle.profile.active_pet.growth.growth_balance_version, 1, "replacement pet uses growth balance v1"); eq(grown_cycle.profile.active_pet.growth.stage_started_at, grown_cycle.profile.active_pet.identity.born_at, "replacement growth starts at replacement birth"); eq(_memory_events_by_types(grown_cycle.profile.active_pet.memory.events, ["pet_grew"]).size(), 0, "replacement pet inherits no growth memories"); eq(grown_cycle.profile.active_pet.memory.events.size(), 1, "replacement pet starts with hatch memory only"); eq(grown_cycle.profile.memorials[-1], grown_memorial, "old grown memorial remains unchanged through replacement cycle")
	...
	for egg_state in ["INCUBATING", "READY", "HATCHING"]:
		var v6_egg_profile := DomainStateScript.new_profile("v6:egg:%s" % egg_state, 1000); v6_egg_profile.schema_version = 6; v6_egg_profile.simulation.simulation_version = 5; v6_egg_profile.simulation.erase("growth_balance_version"); v6_egg_profile.active_subject = "EGG"; v6_egg_profile.active_egg = DomainStateScript.new_egg("egg:v6:%s" % egg_state, 1000, 15400, "speckled"); v6_egg_profile.active_egg.schema_version = 6; v6_egg_profile.active_egg.interaction_summary = {"touch_count":4,"last_interacted_at":1200}; v6_egg_profile.active_egg.state = egg_state
		...
		var source_egg: Dictionary = v6_egg_profile.active_egg.duplicate(true); var migrated_v6_egg: Dictionary = SaveMigratorScript.migrate(v6_egg_profile); eq(migrated_v6_egg.schema_version, 8, "v6 %s root migrates v8" % egg_state); eq(migrated_v6_egg.active_egg.state, egg_state, "v6 %s lifecycle state preserved" % egg_state); eq(migrated_v6_egg.active_egg.schema_version, 8, "v6 %s egg schema migrates" % egg_state)
		for egg_field in ["egg_id", "received_at", "hatch_ready_at", "shell_variant", "interaction_summary", "state"]: eq(migrated_v6_egg.active_egg.get(egg_field), source_egg.get(egg_field), "v6 %s preserves %s" % [egg_state, egg_field])
		if egg_state == "HATCHING":
			for reservation_field in ["reserved_pet_id", "reserved_pet_seed", "hatching_started_at"]: eq(migrated_v6_egg.active_egg.get(reservation_field), source_egg.get(reservation_field), "v6 hatching preserves %s" % reservation_field)
	...
	var v6_hatching = PetGameSessionScript.new(); v6_hatching.balance = BALANCE; v6_hatching.lifecycle = care_session.lifecycle; v6_hatching.care = care_session.care; v6_hatching.survival = SURVIVAL; v6_hatching.relationship_balance = RELATIONSHIP; v6_hatching.memory_config = MEMORY; v6_hatching.growth = GROWTH; v6_hatching.profile = SaveMigratorScript.migrate(v6_hatching_profile); ok(v6_hatching.complete_hatching(16000, 10.0), "migrated v6 hatching completes"); eq(v6_hatching.profile.active_pet.identity.pet_id, "pet:v6:reserved", "migrated hatching keeps reserved identity"); eq(v6_hatching.profile.active_pet.schema_version, 8, "migrated hatching creates schema v8 pet"); eq(v6_hatching.profile.active_pet.life.growth_stage, "NEWBORN", "migrated hatching creates newborn"); eq(v6_hatching.profile.active_pet.growth.growth_version, 1, "migrated hatching creates growth v1"); eq(v6_hatching.profile.active_pet.growth.growth_balance_version, 1, "migrated hatching creates growth balance v1"); eq(v6_hatching.profile.active_pet.growth.stage_started_at, 16000, "migrated hatching newborn growth starts at birth"); eq(v6_hatching.profile.active_pet.memory.events.size(), 1, "migrated hatching starts one hatch memory"); eq(_memory_events_by_types(v6_hatching.profile.active_pet.memory.events, ["pet_grew"]).size(), 0, "hatching creates no growth memory at birth")
	# JSON persistence must canonicalize exact whole growth integers and reject
	# every fractional value rather than accepting a lossy conversion.
	var whole_growth_external: Dictionary = fixture(); whole_growth_external.active_pet.growth = {"growth_version":1.0,"growth_balance_version":1.0,"stage_started_at":1000.0}; whole_growth_external.simulation.growth_balance_version = 1.0; var canonical_growth_external: Dictionary = SaveMigratorScript.migrate(whole_growth_external)
	ok(DomainStateScript.validate_profile(canonical_growth_external), "exact whole growth JSON values migrate validly"); ok(canonical_growth_external.active_pet.growth.growth_version is int and canonical_growth_external.active_pet.growth.growth_balance_version is int and canonical_growth_external.active_pet.growth.stage_started_at is int and canonical_growth_external.simulation.growth_balance_version is int, "exact whole growth JSON values canonicalize to integers")
	for fractional_growth_value in [1.5, 1.000001, -0.000001]:
		for fractional_growth_field in ["growth_version", "growth_balance_version", "stage_started_at"]:
			var fractional_growth_profile: Dictionary = fixture(); fractional_growth_profile.active_pet.growth[fractional_growth_field] = fractional_growth_value; var migrated_fractional_growth: Dictionary = SaveMigratorScript.migrate(fractional_growth_profile); eq(migrated_fractional_growth.active_pet.growth[fractional_growth_field], fractional_growth_value, "%s fractional %s is not canonicalized" % [fractional_growth_field, str(fractional_growth_value)]); ok(not DomainStateScript.validate_profile(migrated_fractional_growth), "%s fractional %s fails profile validation" % [fractional_growth_field, str(fractional_growth_value)]); ok(not LocalSaveRepositoryScript.save_profile(migrated_fractional_growth), "%s fractional %s cannot save" % [fractional_growth_field, str(fractional_growth_value)])
	var fractional_simulation_growth: Dictionary = fixture(); fractional_simulation_growth.simulation.growth_balance_version = 1.5; var migrated_fractional_simulation: Dictionary = SaveMigratorScript.migrate(fractional_simulation_growth); eq(migrated_fractional_simulation.simulation.growth_balance_version, 1.5, "fractional simulation growth version is not canonicalized"); ok(not DomainStateScript.validate_profile(migrated_fractional_simulation), "fractional simulation growth version fails validation"); ok(not LocalSaveRepositoryScript.save_profile(migrated_fractional_simulation), "fractional simulation growth version cannot save")
	var child_round_trip: Dictionary = fixture(); child_round_trip.active_pet.life.growth_stage = "CHILD"; child_round_trip.active_pet.growth = {"growth_version":1,"growth_balance_version":1,"stage_started_at":173800}; child_round_trip.simulation.growth_balance_version = 1; _reset(); ok(LocalSaveRepositoryScript.save_profile(child_round_trip), "valid child growth profile saves"); var loaded_child_round_trip: Dictionary = LocalSaveRepositoryScript.load_profile(); ok(DomainStateScript.validate_profile(loaded_child_round_trip), "valid child growth profile loads"); ok(loaded_child_round_trip.active_pet.growth.growth_version is int and loaded_child_round_trip.active_pet.growth.growth_balance_version is int and loaded_child_round_trip.active_pet.growth.stage_started_at is int and loaded_child_round_trip.simulation.growth_balance_version is int, "growth repository round-trip restores strict integer fields")
...
func _phase7_session(profile_value: Dictionary, lifecycle: Dictionary, care: Dictionary, survival: Dictionary, relationship: Dictionary, memory: Dictionary, growth: Dictionary, rules: Dictionary) -> PetGameSession:
...
func _run_phase7_language_tests(lifecycle: Dictionary, care: Dictionary, survival: Dictionary, relationship: Dictionary, memory: Dictionary, growth: Dictionary) -> void:
	...
	var memory_model = preload("res://domain/memory/memory_model.gd")
	eq(int(rules.get("language_rules_version", 0)), 1, "language rules declare v1"); eq(int(rules.get("max_input_chars", 0)), 256, "language rules declare input limit"); eq(int(rules.get("repeat_window_messages", 0)), 8, "language rules declare repeat window"); eq(int(rules.get("familiar_topic_threshold", 0)), 3, "language rules declare familiar topic threshold"); var serialized_rules := JSON.stringify(rules).to_lower(); ok(not serialized_rules.contains("xin chao") and not serialized_rules.contains("choi") and not serialized_rules.contains("toi"), "ruleset contains no Vietnamese aliases")
	for whitespace_input in ["hello   pet", "hello\tpet", "hello\npet", "hello\rpet", "hello\r\npet"]: eq(LanguageModelScript.normalize(whitespace_input), "hello pet", "language normalizes whitespace form")
	eq(LanguageModelScript.normalize("  Hello,   PET! "), "hello pet", "language normalization trims lowercases and separates punctuation")
	var direct_memory: Dictionary = DomainStateScript.new_pet("pet:language:direct", "Language", 1000, 1).memory
	var english: Dictionary = LanguageModelScript.understand("Hello pet", direct_memory, rules); eq(english.intent, "GREETING", "English greeting recognized"); eq(english.reaction, "ACKNOWLEDGE", "English greeting reaction")
	for rule_value in rules.get("intent_rules", []):
		var rule: Dictionary = rule_value; var phrases: Array = rule.get("phrases", []); var classified: Dictionary = LanguageModelScript.understand(String(phrases[0]), direct_memory, rules); eq(classified.intent, String(rule.intent), "%s English rule recognizes deterministically" % rule.intent); eq(classified.sentiment, int(rule.sentiment), "%s English rule has configured sentiment" % rule.intent); eq(classified.reaction, String(rule.reaction), "%s English rule has configured reaction" % rule.intent)
	var priority: Dictionary = LanguageModelScript.understand("Hello, I love you!", direct_memory, rules); eq(priority.intent, "AFFECTION", "higher priority affection wins deterministic overlap")
	var question: Dictionary = LanguageModelScript.understand("What are you doing?", direct_memory, rules); eq(question.intent, "QUESTION", "question fallback recognized"); eq(question.reaction, "CURIOUS", "question fallback reaction")
	var other: Dictionary = LanguageModelScript.understand("orbital pineapple", direct_memory, rules); eq(other.intent, "OTHER", "other fallback recognized"); eq(other.reaction, "LISTENING", "other fallback reaction")
	var ordered_topics: Dictionary = LanguageModelScript.understand("food play sleep pet i", direct_memory, rules); eq(ordered_topics.topics, ["FOOD","PLAY","SLEEP","PET","USER"], "topic extraction uses canonical stable order")
	var this_animal: Dictionary = LanguageModelScript.understand("this animal", direct_memory, rules); ok("USER" not in this_animal.topics, "single-token i does not match inside this"); var display: Dictionary = LanguageModelScript.understand("display something", direct_memory, rules); ok("PLAY" not in display.topics and display.intent != "PLAY_INVITE", "play does not match inside display"); var partial_phrase: Dictionary = LanguageModelScript.understand("good petal", direct_memory, rules); ok(partial_phrase.intent != "PRAISE", "multi-token phrase rejects partial final token")
	var tie_rules: Dictionary = rules.duplicate(true); tie_rules["intent_rules"] = [{"id":"zeta","intent":"PRAISE","priority":99,"phrases":["tie phrase"],"sentiment":1,"reaction":"HAPPY"},{"id":"alpha","intent":"AFFECTION","priority":99,"phrases":["tie phrase"],"sentiment":1,"reaction":"AFFECTIONATE"}]; eq(LanguageModelScript.understand("tie phrase", direct_memory, tie_rules).matched_rule_id, "alpha", "same-priority rules use stable ID tie-break")
	var runtime_memory_config: Dictionary = DefaultMemoryConfigScript.load_config(); eq(runtime_memory_config.memory_version, 2.0, "runtime memory loader resolves v2 resource")
	_reset()
	var spoken := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules)
	var before_vitals: Dictionary = spoken.profile.active_pet.vitals.duplicate(true); var before_relationship: Dictionary = spoken.profile.active_pet.relationship.duplicate(true); var before_survival: Dictionary = spoken.profile.active_pet.survival.duplicate(true); var before_activity: Dictionary = spoken.profile.active_pet.activity.duplicate(true); var before_growth: Dictionary = spoken.profile.active_pet.growth.duplicate(true); var before_personality: Dictionary = spoken.profile.active_pet.personality.duplicate(true)
	var first: Dictionary = spoken.speak_to_pet("  Hello, pet!  ", 10.0)
	ok(first.ok, "awake pet accepts a message"); eq(first.normalized_text, "hello pet", "speak preserves deterministic normalized text"); eq(first.event.event_type, "pet_heard_message", "speak emits canonical language event"); eq(first.event.subject_id, spoken.profile.active_pet.identity.pet_id, "language event uses pet subject"); eq(first.event.occurred_at, 1000, "language event uses simulated timestamp"); eq(first.event.payload.text, "  Hello, pet!  ", "language event preserves original text"); eq(first.event.payload.language_version, 1, "language event declares language schema"); eq(spoken.profile.active_pet.vitals, before_vitals, "speaking changes no vitals"); eq(spoken.profile.active_pet.relationship, before_relationship, "speaking changes no relationship"); eq(spoken.profile.active_pet.survival, before_survival, "speaking changes no survival"); eq(spoken.profile.active_pet.activity, before_activity, "speaking changes no activity"); eq(spoken.profile.active_pet.growth, before_growth, "speaking changes no growth"); eq(spoken.profile.active_pet.personality, before_personality, "speaking changes no personality")
	var language_memory: Dictionary = spoken.profile.active_pet.memory; eq(language_memory.events[-1].category, "LANGUAGE", "language event projects LANGUAGE memory"); eq(language_memory.events[-1].importance, 1, "language memory has low importance"); eq(language_memory.events[-1].valence, 0, "language memory valence follows sentiment"); eq(language_memory.semantic.language.message_count, 1, "language semantic increments message count"); eq(language_memory.semantic.language.intent_counts.GREETING, 1, "language semantic counts intent"); eq(language_memory.routine, DomainStateScript.new_pet("pet:routine", "Routine", 1000, 1).memory.routine, "language changes no care routine counters"); eq(language_memory.semantic.care_interaction_count, 0, "language changes no care semantic count")
	var repeat: Dictionary = spoken.speak_to_pet("hello pet", 10.0); ok(repeat.ok, "repeat message persists"); eq(repeat.memory_cue, "RECOGNIZED_REPEAT", "repeat cue is deterministic"); eq(spoken.profile.active_pet.memory.semantic.language.repeated_message_count, 1, "repeat counter projects once")
	for phrase in ["have some food", "want food", "eat now"]: ok(spoken.speak_to_pet(phrase, 10.0).ok, "food language input persists")
	var familiar: Dictionary = spoken.speak_to_pet("food", 10.0); eq(familiar.memory_cue, "FAMILIAR_TOPIC", "familiar topic cue follows prior retained topic count"); eq(spoken.profile.active_pet.memory.semantic.language.topic_counts.FOOD, 4, "food topic semantic count remains deterministic")
	var events_before_reject: Array = spoken.profile.recent_events.duplicate(true); var memory_before_reject: Dictionary = spoken.profile.active_pet.memory.duplicate(true); eq(spoken.speak_to_pet("   ", 10.0).reason, "EMPTY_MESSAGE", "empty message rejected"); eq(spoken.speak_to_pet("\t\n\r", 10.0).reason, "EMPTY_MESSAGE", "whitespace-only message rejected"); eq(spoken.speak_to_pet("x".repeat(257), 10.0).reason, "MESSAGE_TOO_LONG", "overlong message rejected"); eq(spoken.profile.recent_events, events_before_reject, "rejected messages append no events"); eq(spoken.profile.active_pet.memory, memory_before_reject, "rejected messages append no memory")
	spoken.profile.active_pet.activity = {"state":"SLEEPING","sleep_started_at":1000}; eq(spoken.speak_to_pet("hello", 10.0).reason, "PET_SLEEPING", "sleeping pet rejects language"); spoken.profile.active_pet.activity = {"state":"AWAKE","sleep_started_at":null}; spoken.profile.active_pet.life.life_state = "DEAD"; spoken.profile.active_pet.life.died_at = 1000; spoken.profile.active_pet.life.death_cause = "STARVATION"; spoken.profile.active_pet.vitals.health = 0.0; eq(spoken.speak_to_pet("hello", 10.0).reason, "PET_DEAD", "dead pet rejects language")
	var no_pet := _phase7_session(DomainStateScript.new_profile("profile:no-language-pet", 1000), lifecycle, care, survival, relationship, memory, growth, rules); eq(no_pet.speak_to_pet("hello", 10.0).reason, "NO_PET", "no-pet profile rejects language")
	var critical := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules); critical.profile.active_pet.survival = {"condition":"CRITICAL","critical_started_at":1000}; ok(critical.speak_to_pet("hello pet", 10.0).ok, "critical awake pet still accepts language"); eq(critical.profile.active_pet.survival.condition, "CRITICAL", "language does not rescue critical state")
	var failed := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules); var failed_profile: Dictionary = failed.profile.duplicate(true); LocalSaveRepositoryScript.test_fail_next_primary_replace = true; var failed_result: Dictionary = failed.speak_to_pet("hello pet", 10.0); eq(failed_result.reason, "PERSIST_FAILED", "language candidate failure reports persistence error"); eq(failed.profile, failed_profile, "failed language candidate preserves all authoritative state")
	var old_memory: Dictionary = memory_model.new_memory()
	for old_event in [{"event_id":"evt:old:hatch","event_type":"pet_hatched","occurred_at":1,"payload":{}},{"event_id":"evt:old:feed","event_type":"pet_fed","occurred_at":2,"payload":{"action":"feed","meaningful":true}},{"event_id":"evt:old:grew","event_type":"pet_grew","occurred_at":3,"payload":{}},{"event_id":"evt:old:critical","event_type":"pet_became_critical","occurred_at":4,"payload":{}}]: old_memory = memory_model.project(old_memory, old_event, memory)
	old_memory.memory_version = 1; old_memory.semantic.erase("language")
	var v7_profile: Dictionary = fixture(); v7_profile.schema_version = 7; v7_profile.active_pet.schema_version = 7; v7_profile.active_pet.memory = old_memory.duplicate(true); var expected_old_events: Array = old_memory.events.duplicate(true); var expected_old_routine: Dictionary = old_memory.routine.duplicate(true); var expected_old_semantic: Dictionary = old_memory.semantic.duplicate(true); var expected_old_sequence: int = old_memory.next_sequence; var migrated_v7: Dictionary = SaveMigratorScript.migrate(v7_profile); ok(DomainStateScript.validate_profile(migrated_v7), "v7 pet migrates to valid v8 language memory"); eq(migrated_v7.active_pet.memory.memory_version, 2, "v7 migration upgrades memory schema"); eq(migrated_v7.active_pet.memory.events, expected_old_events, "v7 migration preserves non-language records exactly"); eq(migrated_v7.active_pet.memory.routine, expected_old_routine, "v7 migration preserves routine projection"); eq(migrated_v7.active_pet.memory.semantic.care_interaction_count, expected_old_semantic.care_interaction_count, "v7 migration preserves care semantic projection"); eq(migrated_v7.active_pet.memory.next_sequence, expected_old_sequence, "v7 migration preserves sequence"); eq(migrated_v7.active_pet.memory.semantic.language.message_count, 0, "v7 migration initializes language projection without backfill")
	var v7_dead_snapshot: Dictionary = fixture().active_pet; v7_dead_snapshot.schema_version = 7; v7_dead_snapshot.memory = old_memory.duplicate(true); v7_dead_snapshot.life.life_state = "DEAD"; v7_dead_snapshot.life.died_at = 1000; v7_dead_snapshot.life.death_cause = "STARVATION"; v7_dead_snapshot.vitals.health = 0.0
	var v7_memorial: Dictionary = DomainStateScript.new_profile("profile:v7-language-memorial", 1000); v7_memorial.schema_version = 7; v7_memorial.memorial_count = 1; v7_memorial.memorials = [{"schema_version":1,"memorial_id":"memorial:v7-language","memorialized_at":1000,"pet_snapshot":v7_dead_snapshot}]; var migrated_v7_memorial: Dictionary = SaveMigratorScript.migrate(v7_memorial); ok(DomainStateScript.validate_profile(migrated_v7_memorial), "v7 memorial snapshot migrates to valid language memory"); eq(migrated_v7_memorial.memorials[0].pet_snapshot.memory.semantic.language.message_count, 0, "v7 memorial migration creates no language backfill")
	var v7_hatching: Dictionary = DomainStateScript.new_profile("profile:v7-language-egg", 1000); v7_hatching.schema_version = 7; v7_hatching.active_subject = "EGG"; v7_hatching.active_egg = DomainStateScript.new_egg("egg:v7-language", 1000, 15400); v7_hatching.active_egg.schema_version = 7; v7_hatching.active_egg.state = "HATCHING"; v7_hatching.active_egg.reserved_pet_id = "pet:v7-language"; v7_hatching.active_egg.reserved_pet_seed = 7; v7_hatching.active_egg.hatching_started_at = 15400; var migrated_v7_hatching: Dictionary = SaveMigratorScript.migrate(v7_hatching); eq(migrated_v7_hatching.active_egg.reserved_pet_id, "pet:v7-language", "v7 hatching preserves reserved identity"); var migrated_hatching_session := _phase7_session(migrated_v7_hatching, lifecycle, care, survival, relationship, memory, growth, rules); ok(migrated_hatching_session.complete_hatching(16000, 10.0), "migrated v7 hatching completes"); eq(migrated_hatching_session.profile.active_pet.identity.pet_id, "pet:v7-language", "migrated hatching keeps reservation identity"); eq(migrated_hatching_session.profile.active_pet.memory.semantic.language.message_count, 0, "migrated hatching creates fresh language memory")
	for egg_state in ["INCUBATING", "READY"]:
		var v7_egg: Dictionary = DomainStateScript.new_profile("profile:v7:%s" % egg_state, 1000); v7_egg.schema_version = 7; v7_egg.active_subject = "EGG"; v7_egg.active_egg = DomainStateScript.new_egg("egg:v7:%s" % egg_state, 1000, 15400, "speckled"); v7_egg.active_egg.schema_version = 7; v7_egg.active_egg.state = egg_state; v7_egg.active_egg.interaction_summary = {"touch_count":2,"last_interacted_at":1200}; var expected_v7_egg: Dictionary = v7_egg.active_egg.duplicate(true); expected_v7_egg.schema_version = 8; var migrated_v7_egg: Dictionary = SaveMigratorScript.migrate(v7_egg); eq(migrated_v7_egg.active_egg, expected_v7_egg, "v7 %s egg fields remain unchanged except schema" % egg_state)
	var malformed_language: Dictionary = migrated_v7.duplicate(true); malformed_language.active_pet.memory.semantic.language.intent_counts.erase("GREETING"); ok(not DomainStateScript.validate_profile(malformed_language), "language semantic missing canonical intent fails validation")
	var mapping_session := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules); ok(mapping_session.speak_to_pet("hello pet", 10.0).ok, "mapping fixture creates valid language memory")
	for malformed_field in [["category", "CARE"], ["importance", 2], ["valence", 1]]:
		var malformed_mapping: Dictionary = mapping_session.profile.duplicate(true); malformed_mapping.active_pet.memory.events[0][malformed_field[0]] = malformed_field[1]; ok(not DomainStateScript.validate_profile(malformed_mapping), "language record %s mapping mismatch fails validation" % malformed_field[0])
	var malformed_payload: Dictionary = mapping_session.profile.duplicate(true); malformed_payload.active_pet.memory.events[0].details.sentiment = 2; ok(not DomainStateScript.validate_profile(malformed_payload), "invalid retained language sentiment fails validation")
	var malformed_rule_id: Dictionary = mapping_session.profile.duplicate(true); malformed_rule_id.active_pet.memory.events[0].details.matched_rule_id = []; ok(not DomainStateScript.validate_profile(malformed_rule_id), "container matched rule ID fails validation")
	var inside_window: Dictionary = memory_model.new_memory(); inside_window = memory_model.project(inside_window, _language_event("evt:cue:a", "hello pet", 1), memory)
	for index in range(7): inside_window = memory_model.project(inside_window, _language_event("evt:cue:inside:%d" % index, "unique inside %d" % index, index + 2), memory)
	eq(LanguageModelScript.understand("hello pet", inside_window, rules).memory_cue, "RECOGNIZED_REPEAT", "repeat inside latest eight language records is recognized")
	var outside_window: Dictionary = memory_model.new_memory(); outside_window = memory_model.project(outside_window, _language_event("evt:cue:old", "hello pet", 1), memory)
	for index in range(8): outside_window = memory_model.project(outside_window, _language_event("evt:cue:outside:%d" % index, "unique outside %d" % index, index + 2), memory)
	eq(LanguageModelScript.understand("hello pet", outside_window, rules).memory_cue, "NONE", "repeat outside latest eight language records is not recognized")
	var non_language_window: Dictionary = memory_model.new_memory(); non_language_window = memory_model.project(non_language_window, _language_event("evt:cue:language", "hello pet", 1), memory)
	for index in range(10): non_language_window = memory_model.project(non_language_window, {"event_id":"evt:cue:care:%d" % index,"event_type":"pet_fed","occurred_at":index + 2,"payload":{"action":"feed","meaningful":true}}, memory)
	eq(LanguageModelScript.understand("hello pet", non_language_window, rules).memory_cue, "RECOGNIZED_REPEAT", "non-language records do not shrink repeat window")
	var familiar_memory: Dictionary = memory_model.new_memory(); eq(LanguageModelScript.understand("food", familiar_memory, rules).memory_cue, "NONE", "zero prior topic mentions are unfamiliar")
	for index in range(3):
		familiar_memory = memory_model.project(familiar_memory, _language_event("evt:familiar:%d" % index, "food %d" % index, index + 1, "OTHER", ["FOOD"]), memory)
		if index < 2: eq(LanguageModelScript.understand("food", familiar_memory, rules).memory_cue, "NONE", "one/two prior topic mentions remain unfamiliar")
	eq(LanguageModelScript.understand("food", familiar_memory, rules).memory_cue, "FAMILIAR_TOPIC", "three prior topic mentions are familiar")
	familiar_memory = memory_model.project(familiar_memory, _language_event("evt:familiar:repeat", "food", 5, "OTHER", ["FOOD"]), memory); eq(LanguageModelScript.understand("food", familiar_memory, rules).memory_cue, "RECOGNIZED_REPEAT", "repeat wins over familiar topic")
	var bounded_language: Dictionary = memory_model.new_memory(); bounded_language = memory_model.project(bounded_language, {"event_id":"evt:language:hatch","event_type":"pet_hatched","occurred_at":1,"payload":{}}, memory)
	for index in range(65): bounded_language = memory_model.project(bounded_language, {"event_id":"evt:language:%d" % index,"event_type":"pet_heard_message","occurred_at":index + 2,"payload":{"text":"message %d" % index,"normalized_text":"message %d" % index,"intent":"OTHER","topics":[],"sentiment":0,"reaction":"LISTENING","memory_cue":"NONE","language_version":1,"language_rules_version":1}}, memory)
	eq(bounded_language.events.size(), 64, "bounded language history obeys global cap"); ok(_has_memory_source(bounded_language.events, "evt:language:hatch"), "low-importance language evicts before lifecycle memory"); eq(bounded_language.semantic.language.message_count, 65, "language summaries survive raw eviction"); eq(bounded_language.next_sequence, 66, "language memory sequence stays lifetime monotonic")
	_reset()
	var language_cycle := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules); ok(language_cycle.speak_to_pet("hello pet", 10.0).ok, "language history is persisted before memorial"); var preserved_language_memory: Dictionary = language_cycle.profile.active_pet.memory.duplicate(true); language_cycle.profile.active_pet.life.life_state = "DEAD"; language_cycle.profile.active_pet.life.died_at = 1000; language_cycle.profile.active_pet.life.death_cause = "STARVATION"; language_cycle.profile.active_pet.vitals.health = 0.0; ok(language_cycle.memorialize_pet(10.0).ok, "dead pet with language history memorializes"); eq(language_cycle.profile.memorials[-1].pet_snapshot.memory, preserved_language_memory, "memorial preserves exact language memory and summary"); ok(language_cycle.request_new_egg(10.0).ok, "replacement egg remains explicit after language memorial"); language_cycle.advance_debug(14400, 10.0); ok(language_cycle.begin_hatching(int(language_cycle.profile.simulation.last_simulated_at), 10.0), "replacement language-cycle hatching begins"); ok(language_cycle.complete_hatching(int(language_cycle.profile.simulation.last_simulated_at), 10.0), "replacement language-cycle hatching completes"); eq(language_cycle.profile.active_pet.memory.events.size(), 1, "replacement pet inherits no language records"); eq(language_cycle.profile.active_pet.memory.semantic.language.message_count, 0, "replacement pet starts with fresh language summary")
	var frozen := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules); ok(frozen.speak_to_pet("hello pet", 10.0).ok, "language freeze fixture accepts history"); frozen.profile.active_pet.life.life_state = "DEAD"; frozen.profile.active_pet.life.died_at = 1000; frozen.profile.active_pet.life.death_cause = "STARVATION"; frozen.profile.active_pet.vitals.health = 0.0; var frozen_memory: Dictionary = frozen.profile.active_pet.memory.duplicate(true); frozen.resume_at(9000, 20.0); frozen.advance_debug(604800, 30.0); frozen.resume_at(1000, 40.0); eq(frozen.profile.active_pet.memory, frozen_memory, "dead language memory freezes across resume/debug/backward clock")
	var ui_session := _phase7_session(fixture(), lifecycle, care, survival, relationship, memory, growth, rules); var ui := FoundationScreenScript.new(); ui.session_override = ui_session; ui._ready(); var feed := ui.lifecycle_button("Feed"); ok(ui.language_input.visible and ui.language_send.visible, "awake pet exposes stable local language controls"); ui_session.profile.active_pet.survival = {"condition":"CRITICAL","critical_started_at":1000}; ui.refresh(); ok(ui.language_input.visible, "critical awake pet exposes language controls"); ui_session.profile.active_pet.activity = {"state":"SLEEPING","sleep_started_at":1000}; ui.refresh(); ok(not ui.language_input.visible and not ui.language_send.visible, "sleeping pet hides language controls"); ui_session.profile.active_pet.activity = {"state":"AWAKE","sleep_started_at":null}; ui_session.profile.active_pet.life.life_state = "DEAD"; ui_session.profile.active_pet.life.died_at = 1000; ui_session.profile.active_pet.life.death_cause = "STARVATION"; ui_session.profile.active_pet.vitals.health = 0.0; ui.refresh(); ok(not ui.language_input.visible, "dead awake pet hides language controls"); ui_session.profile = DomainStateScript.new_profile("profile:ui:none", 1000); ui.refresh(); ok(not ui.language_input.visible, "none state hides language controls"); ui_session.profile.memorial_count = 1; ui.refresh(); ok(not ui.language_input.visible, "memorial state hides language controls"); ui_session.profile.active_subject = "EGG"; ui_session.profile.active_egg = DomainStateScript.new_egg("egg:ui", 1000, 15400); ui.refresh(); ok(not ui.language_input.visible, "egg state hides language controls")
	ui_session.profile = fixture(); ui_session.reanchor(10.0); ui.refresh(); feed = ui.lifecycle_button("Feed"); var ui_rebuilds: int = ui.lifecycle_rebuild_count; ui.language_input.text = "hello pet"; ui._speak(); ok(ui.language_diagnostics.text.contains("GREETING"), "language UI renders deterministic diagnostic result"); eq(ui.lifecycle_button("Feed"), feed, "language refresh does not rebuild care controls"); eq(ui.lifecycle_rebuild_count, ui_rebuilds, "language send leaves lifecycle rebuild count stable"); var ui_after_send: Dictionary = ui_session.profile.duplicate(true); ui.refresh(); ui.refresh(); eq(ui_session.profile, ui_after_send, "UI refresh is read-only")
	ui.free(); ui_session.free(); frozen.free(); migrated_hatching_session.free(); mapping_session.free(); language_cycle.free(); critical.free(); no_pet.free(); failed.free(); spoken.free()

func _language_event(event_id: String, text: String, occurred_at: int, intent := "OTHER", topics: Array = []) -> Dictionary:
	return {"event_id":event_id,"event_type":"pet_heard_message","occurred_at":occurred_at,"payload":{"text":text,"normalized_text":LanguageModelScript.normalize(text),"intent":intent,"topics":topics,"sentiment":0,"reaction":"LISTENING","memory_cue":"NONE","language_version":1,"language_rules_version":1}}
...
func _memory_events_by_types(events: Array, types: Array) -> Array:
	...
	return output
func _unique_memory_source_count(events: Array) -> int:
	var sources := {}
	for event in events: sources[String(event.get("source_event_id", ""))] = true
	return sources.size()
func _reset() -> void:
